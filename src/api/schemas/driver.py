...
VEHICLE_ID_RANGE = validate.Range(
    min=1, error="vehicle_id debe ser un entero positivo")
...
def validate_password(pwd: str):
    ...
    if not (PWD_MIN <= len(pwd) <= PWD_MAX):
        raise ValidationError(
            f"La contraseña debe tener entre {PWD_MIN} y {PWD_MAX} caracteres.")
...
class DriverCreateSchema(BaseDriverSchema):
    ...
    profile_photo_path = fields.Str(
        required=False, allow_none=True, validate=validate.Length(max=512))
...
class DriverUpdateSchema(BaseDriverSchema):
    ...
    profile_photo_path = fields.Str(
        allow_none=True, validate=validate.Length(max=512))
